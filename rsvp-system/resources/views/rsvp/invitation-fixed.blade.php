        ...
        <div class="invitation-card">
            <div class="text-center">
                ...
                <div class="alert alert-info mb-4">
                    <small>
                        <strong>Debug:</strong> Token={{ $guest->unique_link_token }}, Status={{ $guest->rsvp_status }}, 
                        Form Action={{ route('rsvp.submit', $guest->unique_link_token) }}
                    </small>
                </div>
                ...
                @if(!session('success'))
                    ...
                    <!-- Separate forms for each option to ensure data is sent correctly -->
                    <div class="d-grid gap-3">
                        <form action="{{ route('rsvp.submit', $guest->unique_link_token) }}" method="POST" style="display: inline;">
                            @csrf
                            <input type="hidden" name="rsvp_status" value="yes">
                            <button type="submit" class="btn rsvp-btn btn-rsvp-yes w-100">
                        ...
                        <form action="{{ route('rsvp.submit', $guest->unique_link_token) }}" method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn rsvp-btn btn-rsvp-no w-100">
                        ...
                        <form action="{{ route('rsvp.submit', $guest->unique_link_token) }}" method="POST" style="display: inline;">
                            ...
                            <button type="submit" class="btn rsvp-btn btn-rsvp-maybe w-100">
                                <i class="bi bi-question-circle"></i> TALVEZ
                            </button>
                        </form>
                    </div>
    ...
    <script>
        ...
        document.querySelectorAll('form').forEach(function(form) {
            form.addEventListener('submit', function(e) {
                const rsvpStatus = form.querySelector('input[name="rsvp_status"]').value;
                const statusText = rsvpStatus === 'yes' ? 'comparecendo' : 
                                 rsvpStatus === 'no' ? 'não comparecendo' : 'talvez comparecendo';
